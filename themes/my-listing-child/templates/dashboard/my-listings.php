...
$listing_count = count($listing_types);

if ($is_super_admin) {
	$wrapper_class = $listing_count > 1 ? 'col-md-2' : 'col-md-4';
} else {
	$wrapper_class = $listing_count < 2 ? 'col-md-6' : 'col-md-4';
}
?>

<?php do_action( 'mylisting/user-listings/before' ) ?>

<div class="row my-listings-tab-con abdul">
	...
	<?php require locate_template( 'templates/dashboard/partials/filter-by-keyword-input.php' ) ?>
    ...
    <div class="col-md-2 add-new-listing">
        <a href="<?php echo home_url('add-listing'); ?>" class="buttons button-2">
            <?php _e( 'Add New Listing', 'my-listing' )?>
        </a>
    </div>
</div>
...
<div class="row my-listings-stat-box">
	<?php
	if ( mylisting()->get( 'stats.show_mylistings_published' ) !== false ) {
	...
	if ( mylisting()->get( 'stats.show_pending_approval' ) !== false ) {
		mylisting_locate_template( 'templates/dashboard/stats/card.php', [
	...
	if ( mylisting()->get( 'stats.show_pending_payment' ) !== false ) {
		mylisting_locate_template( 'templates/dashboard/stats/card.php', [
			'icon' => 'mi info_outline',
	...
	// Recent views card.
	if ( mylisting()->get( 'stats.show_mylistings_expired' ) !== false ) {
		mylisting_locate_template( 'templates/dashboard/stats/card.php', [
			'icon' => 'mi timer',
...
<div id="job-manager-job-dashboard">
	<?php if ( ! $listings ) : ?>
		<div class="no-listings">
            ...
            <div class="info add-new-listing-box">
                <a href="<?php echo home_url('add-listing'); ?>" class="buttons button-2">
					<?php _e( 'Add New Listing', 'my-listing' )?>
                </a>
            </div>
	...
	<?php else : ?>
		<table class="job-manager-jobs">
			<tbody>
            <tr class="item-id-add-listing" id="item-id-add-listing-item">
                <td class="l-type">
                    <a href="<?php echo home_url('add-listing'); ?>" class="add-listing-button">
                        <i class="bb-icon-anchor bb-icon-l"></i>
		                <?php _e( 'Add New Listing', 'my-listing' )?>
                    </a>
                </td>
            </tr>
				...
				<tr class="listing-cell item-id-<?php echo $listing->get_id() ?> item-product-<?php echo $listing->get_product_id()?:'na' ?> item-type-<?php echo $listing->type?$listing->type->get_slug():'na' ?>">
					<td class="l-type">
						<div class="info listing-type">
							<a href="<?php echo esc_url( add_query_arg( 'filter_by_type', $listing->type->get_slug(), $endpoint ) ) ?>">
								<div class="value">
