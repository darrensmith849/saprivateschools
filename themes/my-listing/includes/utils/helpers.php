...
class Helpers {
	...
	public function file_size_validation( $allowed_size, $file_size, $label, $name ) {
		if ( ! empty( $allowed_size ) && is_numeric( $allowed_size ) && absint( $allowed_size ) < $file_size ) {
			throw new \Exception( sprintf(
				_x( '%1$s This attachement <strong>%2$s</strong> exceeds upload size limit <strong>%3$sKB</strong>.', 'File upload', 'my-listing' ),
				$label ? '"'.$label.'"' : '',
				$name,
				$allowed_size
			) );
