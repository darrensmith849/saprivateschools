...
class Explore_Listings extends Query {
	...
	public function run( $request ) {
		...
		// Combine listing types in global listing type
		if ( $type->is_global() && $type->is_global_custom() ) {
		...
		$end_result = min($start_result + $listings->post_count - 1, $result['found_posts']);
		...
		if ( $result['found_posts'] < 1 ) {
			...
			$result['showing'] = sprintf(
				__( 'Showing <b>%d-%d</b> out of <b>%s</b> results', 'my-listing'),
				$start_result,
				$end_result,
