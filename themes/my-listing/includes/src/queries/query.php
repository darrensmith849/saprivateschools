...
class Query {
	...
	public function keyword_search( $search ) {
		...
		if ( apply_filters( 'mylisting/explore/keyword-search/include-listing-id', false ) !== false && is_numeric( $keyword ) ) {
			$conditions[] = "{$wpdb->posts}.ID = " . esc_sql( absint( $keyword ) ) . "";
		}
