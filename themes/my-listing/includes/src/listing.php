...
class Listing {
	...
	public function get_marker( $size = 'thumbnail' ) {
		if ( $this->has_field( 'logo' ) && !apply_filters( 'mylisting\listing\use-default-marker', false ) ) {
			return c27()->get_resized_image( $this->get_field( 'logo' ), $size );
		}

		if ( $this->type && ( $default_marker = $this->type->get_default_marker( $size ) ) ) {
			return apply_filters( 'mylisting\listing\get_marker\default', $default_marker, $this );
		}
		...
		return apply_filters( 'mylisting\listing\get_marker\default', '', $this );
	}
	...
	public function setup_special_keys() {
		$this->special_keys = apply_filters( 'mylisting/special-keys', [
			...
			':calculate-distance'	=> $this->get_distance_from_location(),
			':views'				=> $this->get_listing_views(),
			':uniqueviews'			=> $this->get_listing_views( true )
		], $this );
	}

	public function get_listing_views( $unique = false ) {
		$visits = new \MyListing\Ext\Visits\Visits();
		return $visits->get_visits( [ 'listing_id' => $this->get_id(), 'unique' => $unique ] );
	}

	public function get_distance_from_location() {
		...
		if ( is_singular( 'job_listing' ) ) {
			if ( ! $listing = \MyListing\Src\Listing::get( get_the_ID() ) ) {
	            return null;
	        }

			$settings = $listing->type->get_layout()['similar_listings'];
			$proximity_filter = $listing->type->get_selected_filter( 'proximity' );

			if ( 'proximity' !== $settings['orderby'] ) {
				return false;
			}

			if ( ( $latitude = $listing->get_field( 'location' )[0]['lat'] ) && ( $longitude = $listing->get_field( 'location' )[0]['lng'] ) ) {

				$proximity = absint( $settings['max_proximity'] );
				$lat = (float) $latitude;
				$lng = (float) $longitude;
				$units = 'km';

				if ( $proximity_filter ) {
					$earth_radius = $proximity_filter->get_prop( 'units' ) === 'imperial' ? 3959 : 6371;
					$units = $proximity_filter->get_prop( 'units' ) === 'imperial' ? 'mi' : 'km';
				}
	        }
		} else {
			if ( empty( $_GET['form_data'] ) || ! is_array( $_GET['form_data'] ) ) {
				return false;
			}

			$form_data = $_GET['form_data'];
			if ( empty( $form_data['proximity'] ) || empty( $form_data['lat'] ) || empty( $form_data['lng'] ) || empty( $form_data['search_location'] ) ) {
				return false;
			}
			...
			$units = isset($_GET['proximity_units']) && $_GET['proximity_units'] == 'mi' ? 'mi' : 'km';
			$earth_radius = $units == 'mi' ? 3959 : 6371;
		}
		
		$distance = $this->measure_circle_distance( $lat, $lng, $this->get_locations( 'lat' ), $this->get_locations( 'lng' ), $earth_radius );

		if ( $units == 'km' && $distance < 1 ) {
			$units = 'm';
			$distance = round($distance * 1000, 0);
			return sprintf( '%s%s', number_format_i18n( $distance, 0 ), $units );
		}

		if ( $units == 'mi' && $distance < 0.5 ) {
			$units = 'ft';
			$distance = round($distance * 5280, 0);
			return sprintf( '%s%s', number_format_i18n( $distance, 0 ), $units );
		}

		return sprintf( '%s%s', number_format_i18n( $distance, 2 ), $units );
	}
	...
	function measure_circle_distance( $lat_from, $long_from, $lat_to, $long_to, $earthRadius = 6371 ) {
	...
	public static function get_post_statuses() {
		return [
			...
			'unpublish' => _x( 'Unpublished', 'post status', 'my-listing' ),
