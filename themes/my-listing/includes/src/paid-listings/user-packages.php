...
class User_Packages {
	...
	public function user_package_posts_columns( $columns ) {
		...
		$columns['hide_in_add_listing'] = esc_html__( 'Hide in add listing process', 'my-listing' );
	...
	public function user_package_posts_custom_column(  $column, $post_id  ) {
		switch ( $column ) {
			...
			case 'hide_in_add_listing':
				$claim = get_post_meta( $post_id, '_hide_in_add_listing', true );
				echo $claim ? __( 'Yes', 'my-listing' ) : __( 'No', 'my-listing' );

			case 'is_claimable':
				$claim = get_post_meta( $post_id, '_is_claimable', true );
				echo $claim ? __( 'Yes', 'my-listing' ) : __( 'No', 'my-listing' );
			break;
