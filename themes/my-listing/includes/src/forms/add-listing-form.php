...
class Add_Listing_Form extends Base_Form {
	...
	public function __construct() {
		...
		if ( $this->job_id ) {
			...
			} elseif ( ! in_array( $job_status, apply_filters( 'mylisting/valid-submission-statuses', [ 'preview', 'draft' ] ), true ) ) {
				$this->job_id = 0;
				$this->step = 0;
			}
		}
	...
	public function submit_handler() {
		...
		$post_status = 'preview';
		if ( $_POST['submit_job'] == 'save--no-preview' ) {
			$post_status = 'draft';
		}
	
		// validation passed successfully, update the listing
		$this->save_listing( $this->fields['job_title']['value'], $description, $this->job_id ? '' : $post_status );
	...
	public function validate_fields() {
		...
		foreach ( $this->fields as $key => $field ) {
			...
			if ( isset( $_POST['submit_job'] ) && $_POST['submit_job'] === 'save--no-preview' ) {
				continue;
			}
	...
	protected function save_listing( $post_title, $post_content, $status = 'preview' ) {
		...
		$allow_slug_update = isset($this->listing_type->settings['disable_slug_update']) ? ! $this->listing_type->settings['disable_slug_update'] : true;

		if ( $allow_slug_update ) {
	...
	public function maybe_skip_preview( $steps ) {
		...
		if ( ! empty( $_POST['submit_job'] ) && $_POST['submit_job'] === 'save--no-preview' ) {
