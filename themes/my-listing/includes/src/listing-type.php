...
class Listing_Type {
	...
	public function get_default_marker( $size = 'thumbnail' ) {
		if ( $image = wp_get_attachment_image_src( $this->get_data( 'default_map_marker' ), $size ) ) {
			return $image[0];
		}
	...
	public function are_packages_enabled() {
		return (bool) $this->settings['packages']['enabled'];
	}
	...
	public function get_claim_fields() {
		return $this->settings['claim_form']['used'];
	}
	...
	public function is_global_custom() {
		return (bool) $this->settings['global_custom'];
	}

	public function get_global_types() {
		$result = array();

		foreach ($this->settings['global_types'] as $key => $value) {
			if ($value === true) {
				$result[] = $key;
			}
		}
