...
class Dashboard_Listings_Controller extends Base_Controller {
	...
	protected function hooks() {
		...
		$this->on( 'mylisting/user-listings/handle-action:status', '@change_listing_status' );
		...
		$this->on( 'mylisting/user-listings/actions', '@display_listing_status_action', 30 );
	}
	...
	private function get_template() {
		...
		$allowed_statuses = [ 'publish', 'pending', 'pending_payment', 'expired', 'preview', 'unpublish', 'draft' ];
	...
	protected function change_listing_status( $listing ) {
		$job_id = ! empty( $_REQUEST['job_id'] ) ? absint( $_REQUEST['job_id'] ) : 0;

		if ( $listing->get_id() !== $job_id ) {
			$this->add_notice( _x( 'Invalid request.', 'User Dashboard > Listings > Actions', 'my-listing' ), 'error' );
	    } else {
	    	$post_status = $listing->get_status();

			$this->add_notice( sprintf(
				_x( '%s has been set to %s', 'User Dashboard > Listings > Actions', 'my-listing' ),
				$listing->get_name(),
				$listing->get_status_label()
			), 'message' );
		}
	}
	...
	protected function display_duplicate_action( $listing ) {
    	// Add a filter hook to enable/disable the duplicate button.
		$show_duplicate = apply_filters( 'mylisting/user-listings/show_duplicate_button', true );

		if ( ! in_array( $listing->get_status(), ['publish', 'pending', 'pending_payment', 'expired'], true ) || ! $show_duplicate ) {
			return;
		}
	...
	protected function display_pending_payment_actions( $listing ) {
		...
		if ( ! in_array( $listing->get_status(), [ 'pending_payment', 'preview', 'draft' ], true ) ) {
			return;
		}
		...
		<?php } else {
			...
			printf(
				...
				esc_url( $resume_url ),
				_x( 'Resume', 'User listings dashboard', 'my-listing' )
			);
		}
	}

	protected function display_listing_status_action( $listing ) {
		if ( ! in_array( $listing->get_status(), ['publish', 'unpublish'], true ) ) {
			return;
		}

		$status_name = esc_html__( 'Unpublish', 'my-listing' );
		$class_name = 'unpublish-listing';

		if ( $listing->get_status() == 'unpublish' ) {
			$status_name = esc_html__( 'Publish', 'my-listing' );
			$class_name = "publish-listing";
		}

		printf(
			'<li class="cts-listing-action-status">
			<a href="#" data-listing-id="%d" class="%s">%s</a>
			</li>',
			absint( $listing->get_id() ),
			esc_attr( $class_name ),
			$status_name
