...
class Elementor {
	...
	public function __construct() {
		...
		if ( apply_filters( 'mylisting/custom-elementor-settings-parser', true ) !== false ) {
			add_action( 'elementor/frontend/before_render', [ $this, 'custom_settings_parser' ] );
		}
	}
	...
	public function theme_header_config( $config ) {
	    ...
	    if ( is_singular('job_listing') || is_page_template('templates/content-featured-image.php') || $is_buddypress_profile ) {
	        ...
	        $config['header']['args']['fixed'] = c27()->get_setting('single_listing_header_fixed', true);
	...
	public function custom_settings_parser( $element ) {
		...
		( \Closure::bind( function( $element ) use ( $get_settings ) {
			if ( ! $element->parsed_active_settings ) {
				$settings = class_exists( '\ElementorPro\Plugin' )
