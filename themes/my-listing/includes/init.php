...
mylisting()->boot(
	...
	MyListing\Controllers\Wp_All_Export_Controller::class,
...
add_filter( 'mylisting/before-add-listing-widget', function( $data ) {

	if ( $data['form_section_animation'] !== 'yes' ) {
		update_option( 'ml_form_section_animation', false );
	} else {
		update_option( 'ml_form_section_animation', true );
	}
...
add_filter( 'body_class', function( $classes ) {
    ...
    if ( ! get_option( 'ml_form_section_animation' ) ) {
    	$classes[] = 'ml-disable-form-section-animation';
    }
