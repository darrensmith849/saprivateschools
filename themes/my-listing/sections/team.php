                        ...
                        <div class="img-hover-holder <?php echo $data['overlay_type'] == 'gradient' ? esc_attr( $data['overlay_gradient'] ) : '' ?>"
                            ...
                            <ul class="social-nav">
                                <?php for ($i = 1; $i <= 5; $i++):
                                    ...
                                    $target = isset($member['social_network_link__' . $i]) && $member['social_network_link__' . $i] ? $member['social_network_link__' . $i]['is_external'] : true;
                                    $rel = isset($member['social_network_link__' . $i]) && $member['social_network_link__' . $i] ? $member['social_network_link__' . $i]['nofollow'] : false;
                                    ?>
                                    <?php if ($icon && $url): ?>
                                        <li class="fb-icon">
                                            <a aria-label="<?php echo esc_attr( _ex( 'Social network link', 'Team widget social network links - SR', 'my-listing' ) ) ?>" href="<?php echo esc_url( $url ) ?>" target="<?php echo $target ? '_blank' : '' ?>" rel="<?php echo $rel ? 'nofollow' : '' ?>">
