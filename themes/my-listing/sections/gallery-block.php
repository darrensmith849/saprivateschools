...
if ($data['gallery_item_interface'] == 'WP_IMAGE_OBJECT') {
	foreach ($data['gallery_items'] as $item) {
		$image_quality = ($data['gallery_type'] == 'carousel-with-preview') ? 'full' : 'medium';
			...
			<div class="pf-head">
				...
				<?php if ( count( $gallery_items ) > 2 ): ?>
					<div class="gallery-nav">
						<ul>
							<li>
								<a aria-label="<?php echo esc_attr( _ex( 'Gallery navigation previous', 'Gallery block arrows - SR', 'my-listing' ) ) ?>" href="#" class="gallery-prev-btn">
									<i class="material-icons keyboard_arrow_left"></i>
								</a>
							</li>
							<li>
								<a aria-label="<?php echo esc_attr( _ex( 'Gallery navigation next', 'Gallery block arrows - SR', 'my-listing' ) ) ?>" href="#" class="gallery-next-btn">
					...
					<?php foreach ((array) $gallery_items as $item): ?>
						<?php if ($item['image']['url']): ?>
							<a class="item photoswipe-item"
							   ...
							   caption="<?php echo esc_attr( $item['caption'] ?? '' ) ?>" 
							   description="<?php echo esc_attr( $item['description'] ?? '' ) ?>"
							   >
							</a>
						<?php endif ?>
					<?php endforeach ?>
			...
			<div class="pf-body">
				<div class="gallerySlider car-slider">
					<div class="owl-carousel galleryPreview photoswipe-gallery">
						<?php foreach ((array) $gallery_items as $item): ?>
							<?php if ($item['image']['url']): ?>
								<a aria-label="<?php echo esc_attr( _ex( 'Gallery image thumbnail', 'Gallery block images - SR', 'my-listing' ) ) ?>" class="item photoswipe-item" href="<?php echo esc_url( $item['large_image'] ? $item['large_image']['url'] : $item['image']['url'] ) ?>">
								   <img src="<?php echo esc_url( $item['image']['url'] ) ?>" alt="<?php echo esc_attr( $item['alt'] ?? '' ) ?>" title="<?php echo esc_attr( $item['title'] ?? '' ) ?>" caption="<?php echo esc_attr( $item['caption'] ?? '' ) ?>" description="<?php echo esc_attr( $item['description'] ?? '' ) ?>">
								</a>
							<?php endif ?>
						<?php endforeach ?>
					</div>
