...
foreach ( (array) $options['footer']['sections'] as $section ) {
    ...
    if ( $section['type'] === 'host' ) {
        ...
        <?php foreach ( $related_items as $key => $related_item ):
            ...
            if ( (! ( $related_item = \MyListing\Src\Listing::get( $related_item ) ) || $related_item->get_status() !== 'publish') && get_post_type( $related_item_id ) !== 'post' ) {
                continue;
            }
            if ( get_post_type( $related_item_id ) === 'post' && get_post_status( $related_item_id ) !== 'publish' ) {
                continue;
            }
            ...
            <div class="event-host c27-footer-section">
                <?php if (get_post_type($related_item_id) === 'job_listing'): ?>
                    <a href="<?php echo esc_url( $related_item->get_link() ) ?>">
                        <?php if ( $related_item_logo = $related_item->get_logo() ): ?>
                            <div class="avatar">
                                <img height="30" width="30" src="<?php echo esc_url( $related_item_logo ) ?>" alt="<?php echo esc_attr( $related_item->get_name() ) ?>">
                            </div>
                        <?php endif ?>
                        <span class="host-name"><?php echo $related_item->compile_string( $section['label'] ) ?></span>
                    </a>
                <?php elseif (get_post_type($related_item_id) === 'post'): ?>
                    <a href="<?php echo esc_url( get_permalink($related_item_id) ) ?>">
                        <?php if ( $related_item_logo = c27()->featured_image($related_item_id) ): ?>
                            <div class="avatar">
                                <img height="30" width="30" src="<?php echo esc_url( c27()->featured_image($related_item_id, 'small') ) ?>">
                            </div>
                        <?php endif ?>
                        <span class="host-name"><?php echo get_the_title($related_item_id) ?></span>
                    </a>
                <?php endif ?>
            ...
            <div class="event-host c27-footer-section">
                <a href="<?php echo esc_url( $listing->author->get_link() ) ?>">
                    <?php if ( $avatar = $listing->author->get_avatar() ): ?>
                        <div class="avatar">
                            <img height="30" width="30" src="<?php echo esc_url( $avatar ) ?>" alt="<?php echo esc_attr( $listing->author->get_name() ) ?>">
                        </div>
                    <?php endif ?>
