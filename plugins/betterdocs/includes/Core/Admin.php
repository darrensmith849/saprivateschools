...
class Admin extends Base {
	...
	public function __construct( Container $container, PostType $type, Enqueue $assets, Settings $settings, KBMigration $kbmigration ) {
		...
		add_action( 'wp_ajax_betterdocs_dismiss_black_friday_pointer', [ $this, 'ajax_dismiss_black_friday_pointer' ] );
	}
	...
	public function insert_plugin_links( $links ) {
		...
		if (! is_plugin_active( 'betterdocs-pro/betterdocs-pro.php' ) ) {
			$links[] = '<a href="https://betterdocs.co/bfcm-2025-plugins-wp" target="_blank" style="color: #000; font-weight: bold;">' . __( 'Save 40% Today', 'betterdocs' ) . '</a>';
		}
	...
	public function ajax_dismiss_black_friday_pointer() {
		...
		if ( ! current_user_can( 'manage_options' ) && ! current_user_can( 'edit_docs' ) ) {
			wp_send_json_error( [ 'message' => __( 'Permission denied', 'betterdocs' ) ] );
		}
		...
		if ( empty( $introduction_key ) ) {
			wp_send_json_error( [ 'message' => __( 'Invalid introduction key', 'betterdocs' ) ] );
		}
		...
		wp_send_json_success( [ 'message' => __( 'Pointer dismissed successfully', 'betterdocs' ) ] );
	}
}
